...
Task 7 scope review: branch-vs-main comparison contains only CI, SDD ledger, hook infrastructure, Linux platform files, Linux-only dependency/config, Tauri bundle config, and the Linux PNG icon. No provider endpoint/request/parsing file and no UI file changed. Wayland/layer-shell and provider enablement remain out of scope.

Manual X11 runtime smoke testing remains intentionally manual because GitHub-hosted CI is headless and does not provide the target desktop session. Final clean-head CI gate is still required after ledger/cleanup changes before the PR is declared ready to merge.
